...
def calculate_neg_mean_log_likelihood(
    ...
    for word in words:
        ...
        word_log_likelihood = torch.tensor(0.0)
        ...
        for i in range(len(processed_word) - 1):
            ...
            if first_char in char_to_index and second_char in char_to_index:
                first_idx = char_to_index[first_char]
                second_idx = char_to_index[second_char]

                bigram_prob = bigram_probabilities[first_idx, second_idx]

                if bigram_prob > 0:
                    word_log_likelihood += torch.log(bigram_prob)
                else:
                    word_log_likelihood += torch.log(torch.tensor(1e-10))

        total_log_likelihood += word_log_likelihood
        total_bigrams += len(processed_word) - 1

    mean_log_likelihood = -total_log_likelihood / total_bigrams if total_bigrams > 0 else 0.0
...
def sample_next_character(
    ...
    # TODO
    current_probs = probability_distribution[current_char_index]
